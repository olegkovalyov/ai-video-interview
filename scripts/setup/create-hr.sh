...
# =============================================================================
# Creates HR user, 3 companies, and 3 interview templates (30 questions)
# =============================================================================
...
header "Creating HR User with Companies & Templates"

# Clear previous credentials
rm -f "${SCRIPT_TMP_DIR}/hr-users.txt" "${SCRIPT_TMP_DIR}/hr-credentials.txt"

# =========================================================================
# STEP 1: Create HR User
# =========================================================================
log_step 1 4 "Creating HR user..."
...
log_success "  HR user created (userId: ${USER_ID})"

# =========================================================================
# STEP 2: Assign HR Role
# =========================================================================
log_step 2 4 "Assigning HR role..."
...
if [ "$(echo "$ROLE_RESPONSE" | jq -r '.success')" != "true" ]; then
  log_error "  Failed to assign HR role"
  exit 1
fi

log_success "  HR role assigned"

# =========================================================================
# STEP 3: Create 3 Companies
# =========================================================================
log_step 3 4 "Creating companies..."
...
# =========================================================================
# STEP 4: Create Interview Templates
# =========================================================================
log_step 4 4 "Creating interview templates..."
...
create_template() {
  ...
  if [ "$TEMPLATE_ID" == "null" ] || [ -z "$TEMPLATE_ID" ]; then
    log_error "  Failed to create template: ${title}"
    return 1
  fi
  ...
  log_success "  Created & published: ${title} (${q_count} questions)"
}
...
echo "${HR_EMAIL}" > "${SCRIPT_TMP_DIR}/hr-users.txt"
echo "${HR_EMAIL}:${HR_PASSWORD}:${USER_ID}" > "${SCRIPT_TMP_DIR}/hr-credentials.txt"

separator
log_success "HR setup completed successfully!"
echo ""
echo "HR Credentials:"
echo "  Email: ${HR_EMAIL}"
