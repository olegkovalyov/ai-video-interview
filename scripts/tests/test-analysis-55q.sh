...
# =============================================================================
# AI Analysis Service - Full Integration Test (55 Questions)
# Sends event via Kafka, polls status, fetches result, checks Kafka output
# =============================================================================

COMMON_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
source "${COMMON_DIR}/common.sh"

API_URL="${AI_ANALYSIS_SERVICE}"
POLL_INTERVAL=5
MAX_POLLS=120  # 10 minutes max

header "AI Analysis Service - Full Integration Test (55 Questions)"

require_container "$KAFKA_CONTAINER"
require_service "AI Analysis Service" "$API_URL"
...
echo "Questions:  55 (expected chunks: 4 = 15+15+15+10)"
separator
...
echo "${PARTITION_KEY}:${EVENT_JSON}" | docker exec -i "$KAFKA_CONTAINER" kafka-console-producer \
...
# STEP 2: Start Kafka consumer in background
log_info "Starting Kafka consumer for analysis-events topic..."

KAFKA_OUTPUT_FILE="${SCRIPT_TMP_DIR}/analysis-event-${INVITATION_ID}.json"
...
# STEP 3: Poll status API
log_info "Polling analysis status..."
...
while [ $POLL_COUNT -lt $MAX_POLLS ]; do
  ...
  FOUND=$(echo "$STATUS_RESPONSE" | jq -r '.found')
...
header "KAFKA EVENT (analysis.completed)"

sleep 2
kill $KAFKA_CONSUMER_PID 2>/dev/null || true

if [ -f "${KAFKA_OUTPUT_FILE}" ] && [ -s "${KAFKA_OUTPUT_FILE}" ]; then
  log_success "Event received from analysis-events topic:"
  head -c 500 "${KAFKA_OUTPUT_FILE}"
  echo ""
  echo "..."
  rm -f "${KAFKA_OUTPUT_FILE}"
else
  log_warn "No event captured (may have been published before consumer started)"
  echo "   Check manually: docker exec $KAFKA_CONTAINER kafka-console-consumer --bootstrap-server localhost:9092 --topic analysis-events --from-beginning --timeout-ms 5000"
fi
