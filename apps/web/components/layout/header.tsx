...
function getRoleLabel(roles: string[]): { label: string; style: string } {
  if (roles.includes("admin"))
    return {
      label: "Admin",
      style: "bg-purple-500/30 text-purple-200 border-purple-400/50",
    };
  if (roles.includes("hr"))
    return {
      label: "HR",
      style: "bg-blue-500/30 text-blue-200 border-blue-400/50",
    };
  return {
    label: "Candidate",
    style: "bg-green-500/30 text-green-200 border-green-400/50",
  };
}
              ...
              <DropdownMenu>
                <DropdownMenuTrigger asChild>
                  <button
                    ...
                    className="flex items-center space-x-3 hover:opacity-80 transition-opacity cursor-pointer outline-none"
                  >
                ...
                >
                  <DropdownMenuLabel className="px-3 py-2.5">
                    <div className="flex flex-col space-y-1">
                      <div className="flex items-center justify-between">
                        ...
                        <span
                          className={`px-2 py-0.5 text-xs font-medium rounded-full border ${role.style}`}
                        >
                          {role.label}
                        </span>
                      </div>
                  ...
                  <DropdownMenuItem asChild className="px-3 py-2.5">
                    <Link
                      href="/profile"
                      className="cursor-pointer text-white hover:bg-white/20 transition-colors"
                    >
