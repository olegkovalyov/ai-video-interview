...
describe('RemoveCandidateSkillCommand Integration', () => {
  ...
  describe('Success Cases', () => {
    it('should remove skill from candidate profile', async () => {
      ...
      // Act - Remove skill
      const removeCommand = new RemoveCandidateSkillCommand(
        candidateId,
        skillId,
      );
    ...
    it('should remove multiple skills from same candidate', async () => {
      ...
      const skill2Command = new CreateSkillCommand({
        name: 'RabbitMQ Test Queue',
        slug: 'rabbitmq-remove-test',
        categoryId: null,
        description: null,
        adminId: uuidv4(),
      });
      ...
      const skill3Command = new CreateSkillCommand({
        name: 'Kafka Test Stream',
        slug: 'kafka-remove-test',
        categoryId: null,
        description: null,
        adminId: uuidv4(),
      });
      ...
      // Act - Remove 2 skills
      await commandBus.execute(
        new RemoveCandidateSkillCommand(candidateId, skill1Id),
      );
      await commandBus.execute(
        new RemoveCandidateSkillCommand(candidateId, skill3Id),
      );
    ...
    it('should remove all skills from profile', async () => {
      ...
      await commandBus.execute(
        new RemoveCandidateSkillCommand(candidateId, skill2Id),
      );
    ...
    it('should remove skill with all metadata', async () => {
      ...
      const addCommand = new AddCandidateSkillCommand({
        candidateId,
        skillId,
        description: 'Detailed description with lots of metadata',
  ...
  describe('Error Cases', () => {
    ...
    it('should throw error when removing already removed skill', async () => {
      ...
      const removeAgainCommand = new RemoveCandidateSkillCommand(
        candidateId,
        skillId,
      );
