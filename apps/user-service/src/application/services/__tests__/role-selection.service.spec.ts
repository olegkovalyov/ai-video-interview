...
describe('RoleSelectionService', () => {
  ...
  beforeEach(async () => {
    ...
    service = module.get<RoleSelectionService>(RoleSelectionService);
  });
  ...
  describe('Success Cases', () => {
    it('should select candidate role and create candidate profile', async () => {
      ...
      // Act
      await service.select({ userId: 'user-id-1', role: 'candidate' });
    ...
    it('should select hr role without creating candidate profile', async () => {
      ...
      // Act
      await service.select({ userId: 'user-id-1', role: 'hr' });
    ...
    it('should select admin role without creating candidate profile', async () => {
      ...
      // Act
      await service.select({ userId: 'user-id-1', role: 'admin' });
  ...
  describe('Error Cases', () => {
    it('should throw UserNotFoundException when user not found', async () => {
      ...
      // Act & Assert
      await expect(
        service.select({ userId: 'non-existent-id', role: 'candidate' }),
    ...
    it('should throw error for invalid role', async () => {
      ...
      // Act & Assert
      await expect(
        service.select({ userId: 'user-id-1', role: 'invalid' as any }),
    ...
    it('should throw when role already selected (not pending)', async () => {
      ...
      // Act & Assert
      await expect(
        service.select({ userId: 'user-id-1', role: 'hr' }),
      ).rejects.toThrow('Role has already been selected');
  ...
  describe('Transaction Behavior', () => {
    ...
    it('should not call schedulePublishing if UoW fails', async () => {
      ...
      // Act & Assert
      await expect(
        service.select({ userId: 'user-id-1', role: 'candidate' }),
