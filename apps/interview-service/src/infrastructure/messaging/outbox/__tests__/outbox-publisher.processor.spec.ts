...
jest.mock('@repo/shared', () => ({
  ...
  KAFKA_TOPICS: { INTERVIEW_EVENTS: 'interview-events' },
...
describe('OutboxPublisherProcessor', () => {
  ...
  describe('publishOutboxEvent', () => {
    ...
    it('should publish to interview-events topic with partitionKey', async () => {
    ...
    it('should re-throw error if retryCount < RETRY_ATTEMPTS (for Bull retry)', async () => {
      ...
      expect(mockLogger.error).toHaveBeenCalledWith(
        expect.stringContaining(
          'Failed to publish test-event-id: Connection timeout',
        ),
        expect.any(String),
      );
    });
    ...
    it('should log error details on Kafka failure', async () => {
      ...
      expect(mockLogger.error).toHaveBeenCalledWith(
        expect.stringContaining(
          'Failed to publish test-event-id: Network error',
        ),
        expect.any(String),
      );
    });
