...
describe('OutboxSchedulerService', () => {
  ...
  describe('pollPendingEvents', () => {
    it('should find pending events and add them to queue', async () => {
      const pendingEvents = [
        { eventId: 'evt-1', eventType: 'invitation.completed' },
        { eventId: 'evt-2', eventType: 'template.created' },
      ];
    ...
    it('should handle "job already exists" errors gracefully', async () => {
      ...
      expect(mockLogger.error).not.toHaveBeenCalledWith(
        expect.stringContaining('Failed to queue event evt-1'),
      );
    });
    ...
    it('should log error for non-duplicate job failures', async () => {
      ...
      expect(mockLogger.error).toHaveBeenCalledWith(
        expect.stringContaining(
          'Failed to queue event evt-1: Redis connection lost',
        ),
      );
    });
    ...
    it('should reset isPolling on error', async () => {
      ...
      expect(mockLogger.error).toHaveBeenCalledWith(
        expect.stringContaining('Outbox polling failed: DB connection lost'),
        expect.any(String),
      );
    ...
    it('should log the count of pending events found', async () => {
      const pendingEvents = [
        { eventId: 'evt-1', eventType: 'invitation.completed' },
        { eventId: 'evt-2', eventType: 'template.created' },
        { eventId: 'evt-3', eventType: 'invitation.started' },
      ];
      mockOutboxRepository.find.mockResolvedValue(pendingEvents);
      ...
      expect(mockLogger.info).toHaveBeenCalledWith(
        expect.stringContaining('Found 3 pending outbox events'),
      );
    });
  });
  ...
  describe('pollStuckEvents', () => {
    ...
    it('should log warning when stuck events are found', async () => {
      ...
      expect(mockLogger.warn).toHaveBeenCalledWith(
        expect.stringContaining('Found 1 stuck outbox events'),
      );
    });
    ...
    it('should handle errors gracefully', async () => {
      ...
      expect(mockLogger.error).toHaveBeenCalledWith(
        expect.stringContaining('Stuck event recovery failed: DB timeout'),
        expect.any(String),
      );
    });
  });
  ...
  describe('cleanupOldEvents', () => {
    it('should delete old published events', async () => {
      ...
      expect(mockLogger.info).toHaveBeenCalledWith(
        expect.stringContaining('Cleaned up 15 old published outbox events'),
      );
    });
