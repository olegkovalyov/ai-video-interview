 ...
 * 2. Publishes to analysis-events topic
 * 3. Updates outbox status
 *
 * ALL ai-analysis-service events go to KAFKA_TOPICS.ANALYSIS_EVENTS.
 */
...
export class OutboxPublisherProcessor {
  ...
  async publishOutboxEvent(job: Job) {
    ...
    this.logger.log(`Publishing outbox event: ${eventId}`);
    ...
    try {
      // 3. Publish to Kafka — ALL analysis-service events go to analysis-events
      ...
      this.logger.log(
        `Published ${eventId} (${outbox.eventType}) to ${KAFKA_TOPICS.ANALYSIS_EVENTS}`,
      );
    } catch (error) {
      // 5. Handle failure
